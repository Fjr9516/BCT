...
% mean subtraction
meanXs = mean(Xs, 2);%整体的均值？？
meanXt = mean(Xt, 2);
Xs = bsxfun(@minus, Xs, meanXs);%减去均值
Xt = bsxfun(@minus, Xt, meanXt);

% L2 normalization
Xs = bsxfun(@times, Xs, 1./max(1e-12, sqrt(sum(Xs.^2))));%正则化
...
%%求每一类的均值
Ms = bsxfun(@rdivide, Xs * idxsi, sum(idxsi)+1e-10);%d*c
Mt = bsxfun(@rdivide, Xt * idxti, sum(idxti)+1e-10);
Ms_ = bsxfun(@rdivide, Xs * ~idxsi, sum(~idxsi)+1e-10);%求mst^i\
Mt_ = bsxfun(@rdivide, Xt * ~idxti, sum(~idxti)+1e-10);

Sb = (Ms + Mt - Ms_ - Mt_) / 2;%d*c
v = sum(Ms .* Sb);
u = sum(Ms.^2) .* sum(Mt.^2);
...
model = train(Ys, sparse(double(Xs'*W)), '-s 2 -c 1 -B 1 -q');
score = model.w * [W'*Xt; ones(1, size(Xt, 2))];%c*nt

th = mean(score, 2)';
[confidence, C] = max(score, [], 1);%每一列最大值
